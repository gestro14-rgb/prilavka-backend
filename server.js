...
function toProductDTO(row) {
  return {
    ...
    title: row.title,
    // Происхождение (migrations/059) — отдельная строка под названием на
    // странице товара и в карточке каталога. Раньше география жила внутри
    // title («Арбуз, Дагестан»); теперь название чистое, а регион — здесь.
    // null — не указано, фронт строку просто не рисует.
...
// Город доставки. Геокодер без города ищет по всей стране и на коротком
// вводе вроде «Профсоюзная улица, 132» уверенно отдаёт другой регион
// (проверено 2026-09-07: Нальчик, Кабардино-Балкарская Республика) — адрес
// признавался вне зоны, хотя в Москве такой дом есть. Все зоны доставки
// московские, поэтому запрос всегда привязываем к городу.
const GEOCODER_CITY = 'Москва';

// Прямое геокодирование: текстовый адрес → координаты.
// Город дописываем сами, если пользователь его не указал: подсказка в поле
// («напр. Профсоюзная, 42») предлагает вводить без города, и без префикса
// такой ввод не находится. Обратного геокодирования (координаты → адрес)
// это не касается — там город приходит из самой точки.
async function geocodeAddress(address) {
  const raw = String(address).trim();
  // Без \b: в JS граница слова считается по ASCII, и перед кириллической «М»
  // её нет — /\bмоскв/ не совпадает с «Москва, …» и город дописывался дважды.
  const query = /москв/i.test(raw) ? raw : `${GEOCODER_CITY}, ${raw}`;
  const result = await requestGeocoder(query);
  if (!result) return null;
  return { ...result, formatted: result.formatted || raw };
}
...
app.get('/api/catalog', resolveUserOptional, async (req, res) => {
  try {
    ...
    res.json({
      ...
      // Минимальная сумма заказа — та же настройка, по которой отказывает
      // POST /api/orders (см. getSetting('min_order_total') там). Мини-апп
      // показывает прогресс до минимума на нескольких экранах и раньше
      // держал это число у себя константой: два источника правды, которые
      // расходились при первой же правке в админке.
      minOrderTotal: Number(getSetting('min_order_total')),
      // Потолок оплаты баллами — по той же причине, что и минимальная сумма
      // выше: POST /api/orders режет списание по этой настройке
      // (max_points_spend_percent), а мини-апп до сих пор считал потолок
      // своей константой 0.30. Расходились они молча: после правки процента
      // в админке checkout продолжал обещать старую скидку, а сервер списывал
      // другую — пользователь видел одну сумму, платил другую.
      maxPointsSpendPercent: Number(getSetting('max_points_spend_percent')),
  ...
  try {
    ...
    await query(
      ...
      [
        ...
        // Пустая строка из формы = «стереть происхождение», поле не
        // передали вовсе = оставить как было.
        p.origin !== undefined ? (p.origin?.trim() || null) : (cur.origin || null),
        req.params.id,
      ]
    );
...
// Эмодзи: до трёх штук. Три — это градация из референса («Острое»
// одним, двумя и тремя перчиками), больше в плашку не помещается.
//
// Предел в code units, а не в символах: одно эмодзи занимает от двух до
// семи единиц (🌶️ — четыре вместе с вариационным селектором, 👨‍🌾 — семь
// со склейкой ZWJ). 16 — это три самых длинных перчика с запасом; при
// прежних 8 трёхуровневые бейджи из референса не создавались вовсе.
const BADGE_ICON_MAX = 16;
...
function validateBadgePayload(p, { partial = false } = {}) {
  ...
  if (has('icon') && p.icon != null && String(p.icon).trim().length > BADGE_ICON_MAX) {
    return 'Иконка — не больше трёх эмодзи';
  }
  return null;
}
